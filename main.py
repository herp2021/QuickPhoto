...
v2 — Fixed processing order (bg removal before face crop) + fixed print sheet grid
"""
...
class WorkerThread(QThread):
    ...
    def run(self):
        try:
            ...
            # ✅ FIX: Remove background FIRST on full original image,
            # then detect face & crop. rembg works far better with full context.
            self.progress.emit(30, "Removing background…")
            img = self.processor.remove_background(img)

            self.progress.emit(55, "Detecting face and centering…")
...
class PassportPhotoApp(QMainWindow):
    ...
    def _apply_stylesheet(self):
        self.setStyleSheet("""
            ...
            QPushButton {
                font-size: 14px; padding: 10px 20px; border-radius: 7px;
                border: 1px solid #CCC; background-color: #FFFFFF; color: #222222;
            }
            QPushButton:hover { background-color: #EAF3FB; border-color: #378ADD; color: #1a5276; }
            ...
            QLabel#spec { font-size: 12px; color: #555; background: #EAF3FB; padding: 6px 10px; border-radius: 5px; }
            QRadioButton { font-size: 13px; color: #222222; spacing: 10px; padding: 5px; }
            QRadioButton::indicator { width: 18px; height: 18px; border: 2px solid #AAA; border-radius: 11px; background-color: white; }
            QRadioButton::indicator:checked { background-color: #1a5276; border: 3px solid #EAF3FB; }
            QRadioButton:hover { background-color: #F8F9FA; border-radius: 4px; }
            QRadioButton:checked { color: #1a5276; font-weight: bold; background-color: #EAF3FB; border-radius: 4px; }
            QCheckBox { font-size: 13px; color: #222222; spacing: 8px; padding: 5px; }
            QCheckBox::indicator { width: 18px; height: 18px; border: 2px solid #AAA; border-radius: 4px; background-color: white; }
            QCheckBox::indicator:checked { background-color: #1a5276; border: 3px solid #EAF3FB; }
            QCheckBox:hover { background-color: #F8F9FA; border-radius: 4px; }
            QCheckBox:checked { color: #1a5276; font-weight: bold; background-color: #EAF3FB; border-radius: 4px; }
            QSlider::groove:horizontal { height: 5px; background: #DDD; border-radius: 3px; }
            QSlider::handle:horizontal { background: #1a5276; width: 18px; height: 18px; margin: -7px 0; border-radius: 9px; }
            QSlider::sub-page:horizontal { background: #378ADD; border-radius: 3px; }
            QProgressBar { border: 1px solid #CCC; border-radius: 5px; text-align: center; font-size: 12px; background: #FFF; height: 20px; }
    ...
    def _gather_settings(self):
        if self.size_passport.isChecked():
            size_mode = "passport"; w, h = 827, 1063
        elif self.size_small.isChecked():
            size_mode = "small_20x20"; w, h = 472, 472
        else:
            size_mode = "small_20x30"; w, h = 472, 709
        return {
            "size_mode": size_mode, "target_w": w, "target_h": h,
    ...
    def process_image(self):
        ...
        self.worker = WorkerThread(self.processor, self.original_image_path, self._gather_settings())
    ...
    def _on_finished(self, image, error):
        ...
        if settings["size_mode"] == "passport":
            size_str = "35×45 mm"; cols, rows = 3, 2
        elif settings["size_mode"] == "small_20x20":
            size_str = "20×20 mm"; cols, rows = 3, 3
        else:
            size_str = "20×30 mm"; cols, rows = 3, 3
        n = cols * rows
        self.result_info.setText(f"{size_str}  ·  {image.width}×{image.height} px  ·  600 DPI")
        self.print_sheet_btn.setText(f"🖨   Save Print Sheet  ({n} photos)")
    ...
    def save_image(self):
        if not self.result_image:
            return
        path, _ = QFileDialog.getSaveFileName(self, "Save Passport Photo", "passport_photo.jpg",
                                              "JPEG (*.jpg);;PNG (*.png)")
        if not path:
            return
    ...
    def save_print_sheet(self):
        if not self.result_image:
            return
        path, _ = QFileDialog.getSaveFileName(self, "Save Print Sheet", "passport_print_sheet.jpg",
                                              "JPEG (*.jpg);;PNG (*.png)")
        if not path:
            return
        cols = getattr(self, "_pending_cols", 3)
        rows = getattr(self, "_pending_rows", 2)
        sheet = self.processor.make_print_sheet(self.result_image, cols=cols, rows=rows)
        fmt = "JPEG" if path.lower().endswith(".jpg") else "PNG"
